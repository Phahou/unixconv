...
typedef struct Thm_t {
  ...
  unsigned int i;			//for iteration
  	  	  	  	  	  	  	//fpos_t is not needed bc it goes only once thru
}Tmst;

Tmst* new_Threadedmsort_argv_t(unsigned int* lineno_,unsigned int* highest,const char* filename,FILE* tmp_, unsigned int i_){
